...
const actions = {
  init({ state, commit, rootState }) {
    ...
    return getAll.request().then(({ data }) => {
      ...
      const routes = routeConfig.useBackendDataAsRoute
      ...
      const adminMenuRoutes = getAuthorizedMenus(routes)[0].children;
      //生成经过权限过滤后的菜单，并传递给layout的store
      appMutations.menus(adminMenuRoutes);
  ...
  function addFullPath(routes, basePath = '/') {
    routes.forEach((route) => {
      ...
      route.fullPath = isExternal(route.path)
        ? route.path
        : resolvePath(basePath, route.path);
