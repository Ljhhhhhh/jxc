  ...
  >
    <abstract-form :model="form" :rules="rules">
      ...
      <collapse-card ref="base" header="基础信息">
        ...
        <abstract-form-item label="操作">
          <el-button @click="handleExport">导出销售单</el-button>
        </abstract-form-item>
      </collapse-card>
...
export default {
  ...
  computed: {
    ...
    headerExtra() {
      return [{ title: '单据状态', content: this.getStatus(this.form.status) }];
    },
  },
  ...
  methods: {
    ...
    changeOutboundNum(nv, ov, row, displayTableRow) {
      ...
      if (outboundNum > displayTableRow.remainNum) {
        ...
        return elAlert(
          `${displayTableRow.cname}的出库数量超出销售数量`,
          () => (row.num = ov),
        );
      }
    },
    ...
    validate() {
      ...
      this.form.data = this.displayTableData.reduce(
        (arr, i) => arr.concat(i.data),
        [],
      );
